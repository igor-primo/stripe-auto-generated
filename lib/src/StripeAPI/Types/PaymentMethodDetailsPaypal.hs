...
data PaymentMethodDetailsPaypal = PaymentMethodDetailsPaypal
  { -- | country: Two-letter ISO code representing the buyer\'s country. Values are provided by PayPal directly (if supported) at the time of authorization or settlement. They cannot be set or mutated.
    ...
    paymentMethodDetailsPaypalCountry :: (GHC.Maybe.Maybe (StripeAPI.Common.Nullable Data.Text.Internal.Text)),
...
mkPaymentMethodDetailsPaypal =
  PaymentMethodDetailsPaypal
    { paymentMethodDetailsPaypalCountry = GHC.Maybe.Nothing,
